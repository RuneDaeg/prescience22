...
정적 파일로 축약해 사용합니다. 현재 범위는 중·고등학교 `과학`, `과학 계열`,
`사회(역사 포함)`, `수학` 교과군의 72과목입니다.
...
### 2026-08-03 — 내용 요소의 개념 단위 세분화

- PDF의 큰 과목 제목을 인식해 물리학·생명과학 등 세부 내용 체계가 다른 과목으로 넘어가던 문제 수정
- 공식 내용 요소가 연결되는 과목을 29개에서 86개로 확대
- `중첩과 간섭`, `DNA와 유전자` 같은 복합 항목을 각각의 설명 가능한 개념으로 분리
- `광합성과 세포호흡의 관계`를 `광합성`, `세포호흡`처럼 1분 설명용 키워드로 세분화

### 2026-08-04 — 과학·사회·수학 중심 개편

- 선택 범위를 과학과, 사회과(역사 포함), 수학과 72과목으로 축소
- 국어·예술·외국어·직업계 등 나머지 교과를 선택과 전체 무작위 추첨에서 제외
- 세로로 표기된 사회과 `지식·이해` 표를 읽는 전용 추출 규칙 추가
- 사회과 17과목의 세부 내용 요소 331개를 공식 PDF에서 추가 추출
- 공유 미리보기 제목·설명·이미지를 과학·사회·수학 전용으로 교체

### 2026-08-04 — 물리학 검수 키워드 확정

- 물리학을 11개 세부 파트로 나누고 사용자 확인을 받은 키워드 목록 추가
- 검수 목록은 자동 추출 결과보다 우선하며 파트 이름은 필터로만 사용
- `전기 안전`, `전기 신호 입력 장치`와 일반 물리학 범위를 벗어난 열 관련 항목 제외
- 승인 전에는 반영하지 않고 과목별 확인 후 배포하는 작업 흐름 확립

### 2026-08-04 — 화학 검수 키워드 확정

- 화학을 9개 세부 파트로 분류하고 사용자 확인을 받은 키워드 목록 추가
- 일반 화학 범위를 벗어난 압력 평형 상수, 적정 곡선 등은 후보에서 제외
- 사용자 요청에 따라 용액 희석·제조, 미지 용액 관련 항목과 적정 실험 기구 제외

### 2026-08-04 — 생명과학 검수 키워드 확정

- 생명과학을 9개 세부 파트로 분류하고 사용자 확인을 받은 키워드 목록 추가
- 탐구 활동명 대신 생명 시스템, 항상성, 유전, 진화와 분류의 독립적인 설명 개념으로 세분화
- 사용자 요청에 따라 `유전적 재조합`을 `유전자 재조합`으로 수정하고 분류 단계 `역`, `계`, `문` 제외

### 2026-08-04 — 지구과학 검수 키워드 확정

- 지구과학을 대기·해양, 암석·지질시대, 태양계·우주의 10개 세부 파트로 분류
- 사용자 확인을 받은 해수 순환, 기상, 지질, 천문 개념 키워드 목록 추가
- 교육과정 범위를 벗어나는 천구 좌표계, 상세 핵반응 메커니즘, 표준 우주 모형 세부 내용 제외

### 2026-08-04 — 발표 집중형 화면 개편

- 과목·파트 선택과 키워드 추첨을 하나의 간결한 시작 화면으로 재구성
- 추첨 시 키워드가 빠르게 바뀌다가 최종 주제에 멈추는 애니메이션 추가
- 조사와 설명 단계에서는 선택 UI를 숨기고 키워드와 대형 타이머에 집중하도록 전체 화면 전환
- 기존의 10분 조사 후 1분 설명 자동 전환, 일시정지, 건너뛰기, 키보드 단축키 유지
- 참고 서비스와 시각적 인상이 겹치지 않도록 검정·주황 조합을 남색·라벤더·민트·아이보리 조합으로 교체
- 대칭형 단일 광원 대신 비대칭 이중 광원과 옅은 학습 노트 격자 배경을 적용해 고유한 시각 체계 구축
...
### 2026-08-04 · 통합사회1 상세 키워드와 교사 직접 추가 기능

- 통합사회1 성취기준과 해설을 바탕으로 13개 세부 파트의 설명형 키워드를 검수 확정 목록에 추가
- 단순 단원명 대신 비교, 인과관계, 변화 양상, 권리와 책임이 드러나는 구체적인 표현을 우선 사용
- 과목 선택 화면에 `＋ 내 키워드` 관리 영역 추가
- 쉼표 또는 줄바꿈으로 여러 키워드를 한꺼번에 입력하고, 개별 삭제하거나 선택 과목의 직접 추가 목록을 전체 삭제할 수 있도록 구현
- 직접 추가한 항목은 선택 과목의 `직접 추가` 파트에 나타나며 기본 교육과정 키워드와 함께 무작위 추첨에 포함

#### 교사 직접 추가 키워드의 저장 방식

직접 추가한 키워드는 서버나 Git 저장소가 아니라 웹 브라우저의 `localStorage`에 저장됩니다. 개인정보나 별도 로그인이 필요 없고 새로고침 후에도 유지되지만, 다음 사항에 유의해야 합니다.

- 같은 기기와 같은 브라우저에서만 목록이 유지됩니다.
- 시크릿 창, 다른 브라우저, 다른 컴퓨터와 자동 동기화되지 않습니다.
- 브라우저 사이트 데이터나 저장 공간을 삭제하면 직접 추가 목록도 삭제됩니다.
- 모든 이용자에게 제공할 공통 키워드는 `sources/curated-keywords.json`에 추가하고 `npm run sync:curriculum -- <원본 저장소 경로>`로 생성 데이터를 갱신한 뒤 검수·커밋해야 합니다.

이 방식은 교사 개인의 수업용 임시·보충 키워드와 프로젝트가 배포하는 공식 검수 목록을 분리하기 위한 것입니다. 여러 교사가 같은 목록을 여러 기기에서 공유해야 한다면 향후 계정 또는 공유 코드 기반 저장 기능을 별도로 설계해야 합니다.
