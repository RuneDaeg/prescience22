...
1. 학교급, 교과군, 과목을 선택합니다.
2. 선택한 과목의 성취기준 요약에서 만든 핵심 과제 한 개를 무작위로 뽑습니다.
...
모든 과목은 동일하게 성취기준 기반으로 동작합니다. 원본 저장소의 `standards.json`에 있는
출처 확인된 기계적 파생 요약을 사용하며, 과목별 영역이 고르게 섞이도록 최대 24개를 추립니다.
화면에는 짧게 다듬은 핵심 과제명과 성취기준 코드·영역·전체 요약을 함께 표시합니다.
...
4. 빌드와 과목 선택 동작을 검증한 뒤 함께 커밋합니다.

생성 파일은 직접 편집하지 않습니다. 성취기준 키워드 생성 규칙과 과목별 최대 개수는
`scripts/sync-curriculum.mjs`에서 관리합니다.
