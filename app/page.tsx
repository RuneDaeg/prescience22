...
type Category = "힘과 에너지" | "전기와 자기" | "빛과 물질";
...
export default function Home() {
  ...
  const filtered = useMemo(
    () => category === "전체" ? KEYWORDS : KEYWORDS.filter((item) => item.category === category),
    [category],
  );
  ...
  return (
    <main className="site-shell">
      <header className="topbar">
        <a className="brand" href="#top" aria-label="피직스 픽 처음으로">
          <span className="brand-mark" aria-hidden="true">P!</span>
          <span>PHYSICS PICK</span>
        </a>
        <div className="curriculum-tag">2022 개정 교육과정 · 물리학</div>
        <div className="round-chip">ROUND {round.toString().padStart(2, "0")}</div>
      </header>

      <section className="hero" id="top">
        <div>
          <p className="eyebrow"><span /> 10분 탐구, 1분 설명</p>
          <h1>오늘의 물리,<br /><em>딱 1분</em>이면 돼.</h1>
        </div>
        <p className="hero-copy">키워드를 뽑고, 핵심을 찾고,<br />나만의 말로 설명해 보세요.</p>
      </section>

      <section className="workspace" aria-label="물리학 키워드 활동">
        <div className="pick-panel">
          <div className="panel-heading">
            ...
            <span className="count-label">총 {filtered.length}개</span>
          </div>

          <div className="filters" aria-label="영역 선택">
            {(["전체", "힘과 에너지", "전기와 자기", "빛과 물질"] as const).map((item) => (
              <button
                key={item}
          ...
          <div className={keyword ? "keyword-card selected" : "keyword-card"} aria-live="polite">
            {keyword ? (
              <>
                <span className="category-pill" style={{ backgroundColor: CATEGORY_COLORS[keyword.category] }}>{keyword.category}</span>
                <div className="keyword-index">KEYWORD {String(KEYWORDS.indexOf(keyword) + 1).padStart(2, "0")}</div>
                <h3>{keyword.name}</h3>
          ...
          <div className="pick-actions">
            <button className="draw-button" onClick={drawKeyword} disabled={running}>
              <span aria-hidden="true">↻</span> {keyword ? "다시 뽑기" : "랜덤 키워드 뽑기"}
            </button>
            {keyword && phase === "ready" && (
              <button className="start-button" onClick={startResearch}>10분 조사 시작 <span aria-hidden="true">→</span></button>
            )}
          </div>
        ...
        <div className={`timer-panel phase-${phase}`}>
          ...
          <div className="stage-track" aria-label="진행 단계">
            <div className={phase === "research" ? "stage current" : phase !== "ready" ? "stage passed" : "stage"}>
              <span>1</span><div><b>자료 조사</b><small>10:00</small></div>
            </div>
            <i />
            <div className={phase === "present" ? "stage current" : phase === "done" ? "stage passed" : "stage"}>
              <span>2</span><div><b>1분 설명</b><small>01:00</small></div>
            </div>
          </div>
          ...
          <div className="timer-wrap">
            <div className="timer-ring" style={{ "--progress": `${progress * 3.6}deg` } as React.CSSProperties}>
              <div className="timer-face">
                ...
                <small>{phase === "ready" ? "키워드를 먼저 뽑아주세요" : running ? "집중해서 핵심을 정리해요" : phase === "done" ? "멋진 설명이었어요!" : "잠시 멈춤"}</small>
              </div>
            </div>
          </div>
          ...
          <div className="timer-actions">
            {phase === "ready" && (
              <button className="primary-timer" onClick={keyword ? startResearch : drawKeyword}>
                {keyword ? "10분 조사 시작" : "키워드 먼저 뽑기"} <span aria-hidden="true">→</span>
              </button>
            )}
            ...
            {phase === "done" && <button className="primary-timer" onClick={nextRound}>다음 키워드 <span aria-hidden="true">→</span></button>}
          </div>
      ...
      <section className="howto" aria-labelledby="howto-title">
        <div>
          <p className="eyebrow"><span /> 수업 활용법</p>
          <h2 id="howto-title">세 단계면<br />충분해요.</h2>
        </div>
        <ol>
          <li><span>01</span><div><b>뽑기</b><p>교육과정 핵심 키워드 중<br />하나를 무작위로 선택해요.</p></div></li>
          <li><span>02</span><div><b>조사하기</b><p>10분 동안 정의·원리·사례를<br />찾아 핵심만 정리해요.</p></div></li>
          <li><span>03</span><div><b>설명하기</b><p>1분 동안 친구에게 가르치듯<br />나만의 말로 설명해요.</p></div></li>
        </ol>
      </section>

      <footer>
        <span>PHYSICS PICK</span>
        <p>2022 개정 교육과정 일반 선택 ‘물리학’ 내용 요소를 바탕으로 구성했습니다.</p>
      </footer>
    </main>
  );
}
